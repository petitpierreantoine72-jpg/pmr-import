...
def get_outliner_selected_node_names():
    """ Returns names of all hidden objects selected in the outliner for Blender 2.83 LTS """
    ...
    for win in bpy.context.window_manager.windows:
        for ar in win.screen.areas:
            if ar.type == 'OUTLINER':
                ...
                break
    if _so is None:
        print('Outliner space not found')
        return []
    ...
    try:
        for tree in subtrees_get(root):
            if tree.select and tree.idcode in {ID_OB, ID_LAYERCOLL}:
                obj_name = tree.name  # Assuming 'tree' has 'name' attribute
                obj_name = obj_name.decode('utf-8')
                if obj_name not in walked:
                    walked.add(obj_name)
                    selected_object_names.append(obj_name)
    except (ValueError, KeyError) as e:
        print(f"Error processing tree element: {e}")
...
def getSelectedNodes():
    ...
    nameList = []
    iterItems = get_outliner_selected_node_names()
