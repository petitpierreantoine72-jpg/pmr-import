...
from .util import logUtil,i3d_binaryUtil,meshchoppingUtil
...
class I3DAnimationSet(object):
    ...
    def _generateClips(self):
        ...
        for actionName in dcc.getActionsByType(self._treeID, "skinning"):
...
class I3DSceneGraph(object):
    ...
    def fixUpI3dSettingsParams(self):
            fixUpI3dParamsFromSetting(self._nodes)
    ...
    def generateShapes(self):
        ...
        dcc.generateMeshOwnershipTables()
 
        boundingVolumes = {}
        for treeID in self.traverse(m_node="DEPTH"):
            treeItem   = self._nodes[treeID]
            data       = treeItem._data
            if ( "I3D_boundingVolume" in data and len(data["I3D_boundingVolume"]) > 0 and data["I3D_boundingVolume"] != "None"):
                boundingVolumes[data["I3D_boundingVolume"]] = treeID
                #dcc.UIAddMessage("generateShapes boundingVolumes[{}]={} added".format(data["I3D_boundingVolume"], treeID))
        ...
        for m_treeID in self.traverse(m_node="DEPTH"):
            ...
            if m_treeID in boundingVolumes.keys():
                ...
                #dcc.UIAddMessage("generateShapes BV overridden: m_treeID({}) m_data[\"boundingVolume\"]={}".format(m_treeID, boundingVolumes[m_treeID]))

            #merge groups
            if(UIGetAttrBool('I3D_exportMergeGroups')): #check if mergeGroups are enabled
                if(I3DGetAttributeValue(m_treeID,"I3D_mergeGroup") != 0):    #mergeGroup
                    if ( "I3D_boundingVolume" not in m_data or len(m_data["I3D_boundingVolume"]) == 0 or m_data["I3D_boundingVolume"] == "None"):
                        mergeGroup = I3DGetAttributeValue(m_treeID,"I3D_mergeGroup")
                        mergeGroupDict[mergeGroup] = mergeGroupDict.setdefault(mergeGroup,[]) + [m_treeID]
                        if(not (mergeGroup in mergeGroupRoot)):
                            mergeGroupRoot[mergeGroup] = m_treeID     #set default root
                        if(I3DGetAttributeValue(m_treeID,"I3D_mergeGroupRoot")):
                            mergeGroupRoot[mergeGroup] = m_treeID     #set choosen root
                            #override BV for root node
                            if ("MERGEGROUP_{}".format(mergeGroup) in boundingVolumes.keys()):
                                m_data["boundingVolume"] = boundingVolumes["MERGEGROUP_{}".format(mergeGroup)]
                                #dcc.UIAddMessage("generateShapes merge group BV set m_data[\"boundingVolume\"]={}".format(boundingVolumes["MERGEGROUP_{}".format(mergeGroup)]))
                        #dcc.UIAddMessage("generateShapes root added {} dict added {}".format(mergeGroupRoot[mergeGroup], m_treeID))
                elif(I3DGetAttributeValue(m_treeID,"I3D_mergeGroup") == 0):  #regular shape
                ...
                if ("TYPE_MESH" == m_data["type"] or "TYPE_NURBS_CURVE" == m_data["type"]):
                    self._shapeID += 1
                    if "I3D_mergeChildren" in m_treeItem._data:
                        m_data["name"] = "MergedChildren{}".format(self._shapeID)
                    m_shapeItem = I3DShapeNode(self._shapeID,m_data)
                    self._shapes[m_shapeItem._treeID] = m_shapeItem

        for boundVolKey in boundingVolumes.keys():
            boundVolumeNode = boundingVolumes[boundVolKey]
            if len(self._nodes[boundVolumeNode]._children) > 0:
                pass
            else:
        ...
        for mergeGroup in mergeGroupRoot.keys():    #iterate through merge groups
            if(len(mergeGroupDict[mergeGroup]) > 1):
                for mergeGroupMemberId in mergeGroupDict[mergeGroup]:
                    if(mergeGroupMemberId == mergeGroupRoot[mergeGroup]):    #root
                        data = self._nodes[mergeGroupMemberId]._data
                        data['mergeGroupNum'] = mergeGroup
                        data['mergeGroupMember'] = []
                        data['skinBindNodeIds'] = []
                        for mergeGroupChildMemberId in mergeGroupDict[mergeGroup]:
                            data['mergeGroupMember'].append(mergeGroupChildMemberId)
                            data['skinBindNodeIds'].append(self._nodes[mergeGroupChildMemberId]._nodeID)
                    else:       #non root
                        self._nodes[mergeGroupMemberId]._data['type'] = "TYPE_TRANSFORM_GROUP"
            else:
                data = self._nodes[mergeGroupDict[mergeGroup][0]]._data

            if ("TYPE_MESH" == data["type"] or "TYPE_NURBS_CURVE" == data["type"]):
                self._shapeID += 1
                m_shapeItem = I3DShapeNode(self._shapeID,data)
                self._shapes[m_shapeItem._treeID] = m_shapeItem

        for m_key, m_shape in self._shapes.items():
            m_shape._generateData()     #access data

        dcc.clearMeshOwnershipTables()
    ...
    def generateFiles(self):
        ...
        for material in self._materials.values():
            ...
            for file, fileType in filesDict.items():
                self._fileID += 1                
                if (0 == file.find("customTexture_")):
                    fileItem = I3DFileNode(self._fileID,fileType,file)
                elif (0 == file.find("customBuffer_")):
                    fileItem = I3DFileNode(self._fileID,fileType,file)
                else:
    ...
    def _xmlWriteSceneObject_TransformGroup(self, treeItem, xmlCurrent):
        ...
        if ( "I3D_lod" in m_data):
            ...
            if ("I3D_lodBlending" in m_data): m_lodBlending = m_data["I3D_lodBlending"]
            ...
            if (m_lodDistance):
                ...
                m_joint = None
                if (m_lodBlending):
                    self._xmlWriteString( xmlCurrent, "lodBlending", "true" )
                else:
                    self._xmlWriteString( xmlCurrent, "lodBlending", "false")
    ...
    def _xmlWriteSceneObject_ShapeMesh(self, treeItem, xmlCurrent):
        ...
        self._xmlWriteAttr( xmlCurrent, "renderedInEnvmap", data, "I3D_renderedInEnvmap" )
        self._xmlWriteAttr( xmlCurrent, "grassMetaDataProvider", data, "I3D_grassMetaDataProvider" )
        self._xmlWriteAttr( xmlCurrent, "grassMesh", data, "I3D_grassMesh" )
        self._xmlWriteAttr( xmlCurrent, "highdDetailMesh", data, "I3D_highdDetailMesh" )
    ...
    def xmlWriteMaterials(self,xmlParent):
        ...
        for key, material in self._materials.items():
            ...
            if "needsRefraction" in data:
                xmlChild = xml_ET.SubElement( xmlCurrent, "Refractionmap" )
                self._xmlWriteString( xmlChild, "type",  "planar" )
                self._xmlWriteString( xmlChild, "coeff", data["refractionCoeff"] )
                self._xmlWriteString( xmlChild, "bumpScale", data["refractionBumpScale"] )
            ...
            if "Custommap" in data:
                for key, m_value in data["Custommap"].items():
                    ...
                    self._xmlWriteString( xmlChild, "name",   key )
                    self._xmlWriteInt(    xmlChild, "fileId", fileID )
            if "CustomBuffer" in data:
                for key, m_value in data["CustomBuffer"].items():
                    xmlChild = xml_ET.SubElement( xmlCurrent, "CustomBuffer" )
                    fileID = self._files[m_value]._fileID
                    self._xmlWriteString( xmlChild, "name",   key )
                    self._xmlWriteInt(    xmlChild, "fileId", fileID )
            ...
            if "doubleSided" in data:
                self._xmlWriteString( xmlCurrent, "doubleSided", data["doubleSided"] )
    ...
    def _xmlWriteShape_Mesh(self, xmlParent, shape):
        ...
        if "dataFormats" in vertices:
            xmlVertices  = xml_ET.SubElement( xmlCurrent, "Vertices" )
            self._xmlWriteString( xmlVertices, "count", vertices["count"]  )
            for key , value in vertices["dataFormats"].items():
                self._xmlWriteString( xmlVertices, key,  "true" )

            for key , value in vertices["dataFormats"].items():
                name = "rtf_" + key
                self._xmlWriteString( xmlVertices, name,  value )
                if key == "tangent" :
                    if UIGetAttrBool("I3D_exportTangentsBinaryRecalc"):
                        self._xmlWriteString( xmlVertices, "tangentrecalculate",  "true" )
                    else:
                        self._xmlWriteString( xmlVertices, "tangentrecalculate",  "false" )

            for m_vert in vertices["data"]:
                xmlV  = xml_ET.SubElement( xmlVertices, "v" )
                for key , value in m_vert.items():
                     self._xmlWriteString( xmlV, key,  value )

        if "extraDataFormats" in vertices:
            xmlVertices  = xml_ET.SubElement( xmlCurrent, "ExtraVertices" )
            for key , value in vertices["extraDataFormats"].items():
                 self._xmlWriteString( xmlVertices, key,  value )

            for m_vert in vertices["extraData"]:
                xmlV  = xml_ET.SubElement( xmlVertices, "v" )
                for key , value in m_vert.items():
                     self._xmlWriteString( xmlV, key,  value )
        ...
        # -------------------------------------------------------------
        if "VertexAnimations" in data and data["VertexAnimations"] != None and len(data["VertexAnimations"]) > 0 :
            xmlVertexAnimations  = xml_ET.SubElement( xmlCurrent, "VertexAnimations" )
            for anim in data["VertexAnimations"]:
                 xmlVertexAnimation  = xml_ET.SubElement( xmlVertexAnimations, "VertexAnimation" )
                 self._xmlWriteString( xmlVertexAnimation, "name", anim["name"]  )
                 numberofFrames = len(anim["keyframes"])
                 vertexCount = len(anim["keyframes"][0])
                 self._xmlWriteString( xmlVertexAnimation, "length", f"{numberofFrames}"  )
                 self._xmlWriteString( xmlVertexAnimation, "vertexCount", f"{vertexCount}"  )
                 for key , value in anim["dataFormats"].items():
                    self._xmlWriteString( xmlVertexAnimation, key,  value )
                 
                 for index, frame in enumerate(anim["keyframes"]):
                      xmlVertexAnimationFrame  = xml_ET.SubElement( xmlVertexAnimation, "Frame" )
                      self._xmlWriteString( xmlVertexAnimationFrame, "id", f"{index}"  )
                      for vert in frame:
                        xmlV  = xml_ET.SubElement( xmlVertexAnimationFrame, "v" )
        ...
        if value != i3dDefaultValue:
            ...
            if TYPE_BOOL == attrType:
                ...
                try:
                    ...
                    self._xmlWriteInt(xmlCurrent, attrStr, asInt)
                except Exception as e:
...
class I3DIOexport( object ):
    ...
    def export( self, exportSelection = False ):
        ...
        try:
            ...
            if UIGetAttrBool("I3D_updateXMLOnExport") and not exportSelection:
                self._updateXML()
            meshchoppingUtil.cleanupAfterExport( self._cleanupList )
            return err
    ...
    def _generateSceneGraph(self, onlyScenegraphNodes=False):
        ...
        maxmeshSizeY = 0; 
        maxmeshSizeZ = 0; 
        if not UIGetAttrBool('I3D_exportMergeGroups') : 
            maxmeshSizeX = UIGetAttrFloat('I3D_UImaxMeshSizeX')
            maxmeshSizeY = UIGetAttrFloat('I3D_UImaxMeshSizeY')
            maxmeshSizeZ = UIGetAttrFloat('I3D_UImaxMeshSizeZ')

        if maxmeshSizeX > 0 or maxmeshSizeY > 0 or maxmeshSizeZ > 0 :
            self._objectsToExportList,self._cleanupList = meshchoppingUtil.process_exportlist(self._objectsToExportList, maxmeshSizeX, maxmeshSizeY, maxmeshSizeZ )
        ...
        self._sceneGraph.fixUpI3dSettingsParams()
    ...
    def _generateSceneGraphItem(self,node,parent):
        ...
        children = dcc.getChildObjects(node)
        for child in children:
            if (not self._exportSelection) or (child in self._objectsToExportList):
    ...
    def _addI3dMapping(self, mappingList, localRoot = "ROOT"):
        ...
        for childNodeStr in self._sceneGraph._nodes[localRoot]._children:
            ...
            xmlIdentifier = dcc.getXMLConfigID(childNodeStr)
            xmlIdentifier = xmlIdentifier.strip()
            indexPath = dcc.getNodeIndex(childNodeStr)
            # print("export: {},indexPath: {}, id: {}".format(dcc.getXMLConfigBool(childNodeStr),dcc.getNodeIndex(childNodeStr),dcc.getXMLConfigID(childNodeStr)))
            if dcc.getXMLConfigBool(childNodeStr):
...
class I3DCollisionFilter:
    def __init__(self, group, mask):
        self.group = int(group)
        self.mask = int(mask)
    
    def __str__(self):
        return f"group({self.group}) mask({self.mask})"


import xml.etree.ElementTree as StandardXML

class I3DCollisionFilters:
    def __init__(self):
        self.readCollisionFiltersXML()
    
    def __str__(self):
        res = "{\n"
        
        for presetName, presetFilter in self.presets.items():
            res += f"    preset[{presetName}] = {{{presetFilter}}}\n"
    ...
    def isValid(myString):
        return type(myString) == str or not (myString and myString.strip())
    
    def readBits(self, xmlPreset, attrName):
        flags = 0
        
        xmlFlags = xmlPreset.find(attrName)
        xmlFlagsAttr = xmlFlags.get("value")
        
        if xmlFlagsAttr is not None:
            flags = int(xmlFlagsAttr)
        else:
            for xmlFlag in xmlFlags.findall("flag"):
                bit = int(self.bits[xmlFlag.get("name")])
                flags |= 1 << bit
        
        return flags
    
    def readCollisionFiltersXML(self):
        tree = StandardXML.parse(os.path.dirname(__file__) + "/collisionMaskFlags.xml")
        root = tree.getroot()
        ...
        for flag in root.findall("flag"):
            self.bits[flag.get("name")] = int(flag.get("bit"))
        
        # read presets
        for xmlPreset in root.findall("preset"):
            presetName = xmlPreset.get("name")
            
            if not self.isValid(presetName):
                continue
            
            presetGroup = self.readBits(xmlPreset, "group")
            presetMask = self.readBits(xmlPreset, "mask")
            
            self.presets.update({ presetName : I3DCollisionFilter(presetGroup, presetMask) })
        
        self.presetsEnum = tuple()
        
        # first insert empty as this is needed for all shapes that does not have the property set
        self.presetsEnum = self.presetsEnum + (("-","",""),)
        
        for key, value in self.presets.items():
            self.presetsEnum = self.presetsEnum + ((key,key,""),)
